...
app_basic_name=$(read_kv_config .env.docker APP_NAME);               # 项目名称
app_env=$(read_kv_config .env.docker APP_ENV);                       # app env dev test prod pre
...
function init_app()
{
    ...
    echo dk_nginx_domain=$nginx_domain >> $project_docker_persistent_dir/config
    echo dk_nginx_root=$dk_nginx_root >> $project_docker_persistent_dir/config
    ...
    run_cmd "replace_template_key_value $project_docker_persistent_dir/config $project_docker_php_dir/config-template/.user.ini $dk_nginx_root/.user.ini"
...
function clean_runtime()
{
    ...
    run_cmd "rm -rf $dk_nginx_root/.user.ini"
}
